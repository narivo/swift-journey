...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if let startURL = Bundle.main.url(forResource: "start", withExtension: "txt") {
            if let contentsOfStart = try? String(contentsOf: startURL) {
                allWords = contentsOfStart.components(separatedBy: "\n")
            }
        }
    ...
    func submit(_ answer: String) {
        ...
        if isReal(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                ...
                alert(title:"Word used already",
                      message:"Be more original!")
            }
        } else {
            alert(title:"Word not recognised",
    ...
    func isReal(word: String) -> Bool {
        ...
        let exists = misspelledRange.location == NSNotFound
        // more than 3 letters & not start word
        let count = word.count
        let notStartWord = word != title?.lowercased()
        
        return exists && count > 3 && notStartWord
    }
