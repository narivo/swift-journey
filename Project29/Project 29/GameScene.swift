...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var windDirection = 0.0 {
        didSet {
            windLabel.text = "Wind direction: \(windDirection)"
        }
    }
    ...
    override func didMove(to view: SKView) {
        ...
        configureWindLabel()
        Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(updateWindLevel), userInfo: nil, repeats: true)
        createBuildings()
        createPlayers()
    ...
    func configureWindLabel() {
        ...
        windLabel.position = CGPoint(x: 512, y: 100)
        windLabel.fontSize = 30
        windLabel.zPosition = 100
        windLabel.fontName = "Chalkduster"
        windLabel.fontColor = UIColor.black
        addChild(windLabel)
    }
    
    @objc func updateWindLevel() {
        windDirection = Double.random(in: -2...2)
        physicsWorld.gravity = CGVector(dx: windDirection, dy: 0)
    }
    ...
    func launch(angle: Int, velocity: Int) {
        ...
        let radians = deg2read(degrees: angle)
    ...
    func deg2read(degrees: Int) -> Double {
    ...
    func destroy(player: SKSpriteNode) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            guard let scorePlayerOne = self.viewController?.scorePlayerOne, let scorePlayerTwo = self.viewController?.scorePlayerTwo else { fatalError() }
            ...
            if scorePlayerOne >= 3 || scorePlayerTwo >= 3 {
                ...
                endGameLabel.text = "End Game player \(self.currentPlayer) WON !"
                self.addChild(endGameLabel)
                self.viewController?.disableInteractions()
            } else {
                let newGame = GameScene(size: self.size)
                newGame.viewController = self.viewController
                self.viewController?.currentScene = newGame
